...
export default function StudentTimetable() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const data = await getLessons(sid, { classId: student.class_id })
      setLessons(data || [])
      setLoading(false)
    }
    load()
  ...
  const subjectColors = {}
  const seen = []
  lessons.forEach(l => { if (l.subject_id && !subjectColors[l.subject_id]) { subjectColors[l.subject_id] = COLORS[seen.length % COLORS.length]; seen.push(l.subject_id) } })
                      ...
                      <td key={day} className="table-td min-w-[120px] align-top py-1">
                        {slot.map(l => (
                          <div key={l.id} className={`rounded-lg px-2 py-2 text-xs mb-1 ${subjectColors[l.subject_id] || COLORS[0]}`}>
                            ...
                            <p className="opacity-70">{l.teachers?.profiles ? `${l.teachers.profiles.first_name} ${l.teachers.profiles.last_name}` : ''}</p>
